...
> Nota local do IC-JPEG: este arquivo é documentação upstream vendorizada do componente `esp32-camera`. Ele serve apenas como referência do driver da câmera; a topologia CAM<->S3, a pinagem local e o protocolo SPI experimental do projeto estão documentados em `prototype/README.md`, `docs/protocol.md` e `docs/FRAME_FORMAT.md`.
>
> Estado local atual: a ESP32-CAM usa OV2640 em `PIXFORMAT_RGB565`, `FRAMESIZE_QVGA`, `320x240`, `XCLK=8 MHz`, `fb_count=1`, `CAMERA_GRAB_WHEN_EMPTY`, PSRAM, AWB/AEC/AGC automaticos, `vflip=0` e `hmirror=0`. O firmware faz warm-up leve no boot e descarta frames adicionais antes da captura real para reduzir a primeira imagem rosada. O enlace com o S3 e SPI `mode 1`, `1 MHz`, chunks de `512 B`.
