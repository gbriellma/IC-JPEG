...
def compute_bitrate(quantized_blocks):
    ...
    total_bits = 0.0
    num_blocks = quantized_blocks.shape[0] if hasattr(quantized_blocks, "shape") else len(quantized_blocks)
...
def results_table(results, total_time, image_name, directory):
    ...
    headers = ["k Factor", "PSNR (dB)", "SSIM", "Bitrate (bpp)",
               "Tempo (ms)", "Comp", "Decomp"]
    rows = []
    for k, psnr, ssim, time_ms, bitrate, comp_ms, decomp_ms in results:
        time_s = time_ms / 1000.0
        time_min = time_s / 60.0
        rows.append([
            f"{k:g}", f"{psnr:.2f}", f"{ssim:.4f}", f"{bitrate:.6f}",
            f"{time_ms:.2f}", f"{comp_ms:.2f}", f"{decomp_ms:.2f}"
        ])
...
def print_results(results, total_time, image_name, output_dir=None, plot_dir=None):
    ...
    lines.append("┌─────────┬───────────┬─────────┬──────────────────────────────┬────────────────────────────┐")
    lines.append("│ k Factor│ PSNR (dB) │  SSIM   │ Bitrate (bits/pixel)       │ Time (ms) [Comp/Decomp]  │")
    lines.append("├─────────┼─────────┼─────────┼──────────────────────────────┼────────────────────────────┤")
    for k, psnr, ssim, time_ms, bitrate, comp_ms, decomp_ms in results:
        time_s = time_ms / 1000.0
        time_min = time_s / 60.0
        lines.append(f"│ {k:>7g}│ {psnr:9.2f} │ {ssim:7.4f} │ {bitrate:18.6f} │ {time_ms:8.2f} [{comp_ms:6.2f}/{decomp_ms:6.2f}] │")
    lines.append("└─────────┴───────────┴─────────┴──────────────────────────────┴────────────────────────────┘")
    total_time_s = total_time / 1000.0
    total_time_min = total_time_s / 60.0
...
def plot_dataset(results_global, bitrate_global, out_dir='plots'):
    ...
    import pandas as pd
    df = pd.DataFrame(flat, columns=['k', 'PSNR', 'SSIM', 'Time', 'Bitrate', 'CompTime', 'DecompTime'])
...
def plot_comp_decomp_time(results, image_name, out_dir='plots'):
    ks = [str(r[0]) for r in results]
    comps = [r[5] for r in results]
    decomps = [r[6] for r in results]
    ...
    plt.barh(y - height/2, comps, height, label='Compressão (ms)', color='skyblue')
    plt.barh(y + height/2, decomps, height, label='Descompressão (ms)', color='salmon')

    plt.xlabel('Tempo (ms)', fontsize=11)
    plt.ylabel('Fator k', fontsize=11)
    plt.title(f'Tempo de Compressão e Descompressão: {name}', fontsize=12)
    plt.yticks(y, ks)
    plt.legend()
    plt.grid(True, axis='x', linestyle='--', alpha=0.6)

    plt.tight_layout()
    plt.savefig(os.path.join(sub, name + '_time_bar.png'), dpi=200)
    plt.close()
